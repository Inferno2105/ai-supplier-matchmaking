...
async def _score_pair(client_doc: dict, supplier_doc: dict) -> tuple[ScoreBreakdown, float, float]:
    ...
    client = ClientCreate(**client_doc)
    supplier = SupplierCreate(**supplier_doc)
...
async def _notify_both_sides(match_id: str, client_id: str, supplier_id: str, score: float):
    ...
    client_msg = NotificationInDB(
        ...
        match_id=match_id,
        message=f"New match found: {supplier_doc['supplier_name']} scored {score}% for your \"{client_doc['product_requirement'][:60]}\" requirement.",
    ).model_dump()
    supplier_msg = NotificationInDB(
        user_id=supplier_doc["user_id"],
        match_id=match_id,
        message=f"New match found: {client_doc['company_name']} scored {score}% for your \"{supplier_doc['product_offered'][:60]}\" offering.",
    ).model_dump()
...
async def run_matching_for_client(client_id: str):
    ...
    suppliers = supplier_profiles.find({"category": client_doc["category"]})
    results = []
...
async def run_matching_for_supplier(supplier_id: str):
    ...
    clients = client_profiles.find({"category": supplier_doc["category"]})
    results = []
