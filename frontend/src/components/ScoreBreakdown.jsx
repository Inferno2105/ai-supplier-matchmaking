...
export default function ScoreBreakdown({ breakdown, quantityFulfilledRatio }) {
  ...
  return (
    <div className="mt-2">
      ...
      {open && (
        <div className="mt-2 space-y-1.5 rounded-lg border border-slate-200 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-900/40">
          {FACTORS.map((f) => (
            <div key={f.key} className="flex items-center gap-2">
              <span className="w-36 shrink-0 text-xs text-slate-600 dark:text-slate-400">
                {f.label} <span className="text-slate-400 dark:text-slate-500">({f.weight})</span>
              </span>
              ...
              <span className="w-12 shrink-0 text-right text-xs font-medium text-slate-700 dark:text-slate-300">
